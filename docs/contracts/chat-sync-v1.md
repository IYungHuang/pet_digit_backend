...
## 7. Media, deletion, and cleanup ownership

Client uploads only original media to:
`rooms/{roomId}/staging/{uid}/{clientId}/original`. Thumbnail upload is not part
of v1 (Option B): client thumbnail fields and thumbnail staging paths are
rejected by Functions and Storage Rules. Future thumbnails must be generated by
trusted backend processing and stored under finalized server-owned paths.

`fileName` is required integrity metadata and must equal Storage custom
metadata. `checksum` is required backend-verified `sha256:<64 lowercase hex>`.
Canonical media stores finalized paths only; staging paths never enter message
contract.

Only message sender may call `removeMessage`; active membership is necessary
but insufficient. The operation writes `state=deleted` tombstone and never
hard-deletes message history. Room admin deletion is not enabled by current
membership schema.

Backend owns scheduled recovery/cleanup: request lease recovery every 5 minutes,
staging TTL cleanup hourly (24h TTL), and finalized orphan cleanup hourly (24h
